$(document).ready(function(){
	$("#post").click(function(){
		...
		$.post("/Devil666/postback",{
			take: $("#textarea").val()});
		var add= parseInt($("#totalCount").val()) + 1;
		$("#totalCount").val(add);
		var id_name = "div"+add;
		$("#wall-comments").prepend('<div style="width: 845px;float: left;margin-top: 5px;margin-left: 20px;" id="'+id_name+'"></div>');
		var image = "image"+add;
		$("#"+id_name).append('<div style=" width: 153px; float: left;" id= "'+image+'"></div>');
		var write = "write"+add;
		$("#"+id_name).append('<div style="background-color: white; width: 80%; float: left;" id= "'+write+'"></div>');
		var pic = "pic"+add;
		$("#"+image).append('<img id="'+pic+'" src="http://desktop.freewallpaper4.me/view/original/5904/jesus-and-the-devil.jpg" width="153px" height="130" />');
		var above = "above"+add;
		$('#'+write).append('<div style= "background-color: blue;height: 20px;width: 100%;float: left; align: left;"id= "'+above+'"></div>');
		$('#'+above).append('<div style= "background-color: #888888;height: 20px;width:85%;float: left; align: left;">RItwik put it into the flame wall on01-07-2014</div>');
		var edit = "edit"+add;
